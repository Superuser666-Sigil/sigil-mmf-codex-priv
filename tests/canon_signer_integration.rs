...
fn test_canon_signer_generate_key() {
    ...
    let output = Command::new("cargo")
        .args([
            "run", "--bin", "canon_signer", "--", 
            "generate-key", "--output", key_path.to_str().unwrap()
        ])
        .output()
    ...
    let _key_json: serde_json::Value = serde_json::from_str(&key_content)
        .expect("Key file is not valid JSON");
}
...
fn test_canon_signer_help() {
    ...
    assert!(help_text.contains("Sign Canon records"), "Help text missing description");
    assert!(help_text.contains("generate-key"), "Help text missing generate-key command");
    assert!(help_text.contains("sign-file"), "Help text missing sign-file command");
}
...
fn test_canon_signer_invalid_args() {
    ...
    // Should fail
    assert!(!output.status.success(), "canon_signer should fail with invalid command");
