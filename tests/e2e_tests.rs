...
async fn create_test_runtime_from_license(
    license_path: &str,
) -> (Arc<RwLock<SigilRuntimeCore>>, TempDir) {
    // Read the license first to get the expected runtime/canon IDs
    let license_content =
        std::fs::read_to_string(license_path).expect("Failed to read license file");
    ...
    let wrapper: TestLicenseWrapper =
        toml::from_str(&license_content).expect("Failed to parse license TOML");
    ...
    println!(
        "🔑 Using license for {} with LOA: {:?}",
        license_result.license.owner.name, loa
    );
...
async fn e2e_license_based_multi_user_root_operations() {
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "Alice (Root license) should be allowed system Canon writes"
    );
    ...
    assert_eq!(
        response_json["allowed"], true,
        "Root license should allow system Canon operations"
    );
    assert!(
        response_json["score"].as_f64().unwrap() > 0.0,
        "Trust score should be positive for Root"
    );
}
...
async fn e2e_license_based_mentor_operations() {
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "Bob (Mentor license) should be allowed system Canon reads"
    );
    ...
    assert_eq!(
        response_json["allowed"], true,
        "Mentor license should allow system Canon reads"
    );
}
...
async fn e2e_license_based_operator_module_execution() {
    ...
    if status != StatusCode::OK {
        println!(
            "Carol (Operator) module execution failed with {}: {}",
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "Carol (Operator license) should be allowed to execute modules"
    );
...
async fn e2e_allow_trust_check_operator() {
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "Operator should be allowed to perform read operations: {}",
        response_text
    );
    ...
    assert_eq!(
        response_json["allowed"], true,
        "Trust evaluation should allow the operation"
    );
    assert!(
        response_json["score"].as_f64().unwrap() > 0.0,
...
async fn e2e_allow_module_execution_operator() {
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "Operator should be allowed to execute modules: {}",
        response_text
    );
...
async fn e2e_allow_canon_user_write_operator() {
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "Operator should be allowed to write to user Canon space"
    );
...
async fn e2e_deny_module_execution_guest() {
    ...
    assert_eq!(
        ...
        "Guest should be denied module execution"
    );
...
async fn e2e_deny_trust_check_guest_risky_action() {
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "Trust check should return status, not deny access"
    );
    ...
    assert_eq!(
        response_json["allowed"], false,
        "Guest should not be allowed canon_write actions"
    );
}
...
async fn e2e_error_default_deny_missing_user_id() {
    ...
    assert!(
        response_text.contains("user_id cannot be empty"),
...
fn e2e_tamper_audit_reasoning_chain_integrity() {
    ...
    finalized_chain
        .finalize_reasoning()
        .expect("Should be able to finalize chain");

    // Freeze the chain directly (without storing in Canon to avoid quorum requirements)
    let frozen = FrozenChain::freeze_reasoning_chain(finalized_chain)
        .expect("Should be able to freeze chain");

    // Verify integrity
    assert!(
        frozen.verify_integrity().is_ok(),
        "Frozen chain should have valid integrity"
    );
...
fn e2e_tamper_audit_canonical_record_integrity() {
    ...
    assert!(
        record.payload.get("audit").is_some(),
        "Should have audit field"
    );
    // The chain_id is in audit.chain_id, not directly in the payload
    let audit = record.payload.get("audit").unwrap();
    assert!(
        audit.get("chain_id").is_some(),
        "Should have chain_id in audit"
    );
    assert!(
        record.payload.get("reasoning").is_some(),
    ...
    let canonical_json = record
        .to_canonical_json()
        .expect("Should be able to generate canonical JSON");
