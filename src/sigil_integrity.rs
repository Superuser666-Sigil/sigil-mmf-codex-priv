...
fn load_witness_key(path: &str) -> Result<VerifyingKey, String> {
    let key_bytes =
        std::fs::read(path).map_err(|e| format!("Failed to load witness key from {path}: {e}"))?;
...
pub fn initialize_witness_registry() -> SigilResult<()> {
    let mut registry = WITNESS_REGISTRY.lock().map_err(|_| {
        crate::errors::SigilError::internal("Failed to acquire witness registry lock")
    })?;
...
pub fn get_witness_count() -> SigilResult<usize> {
    let registry = WITNESS_REGISTRY.lock().map_err(|_| {
        crate::errors::SigilError::internal("Failed to acquire witness registry lock")
    })?;
...
pub fn validate_witnesses(
    ...
    if witnesses.len() < 3 {
        log::warn!(
            "Witness quorum not satisfied: {} witnesses, minimum 3 required",
            witnesses.len()
        );
        return Ok(false);
    }
    ...
    if witnesses.len() > 10 {
        log::warn!(
            "Too many witnesses: {} witnesses, maximum 10 allowed",
            witnesses.len()
        );
        return Ok(false);
    }
    ...
    for witness in witnesses {
        ...
        if let Some(public_key) = registry.get(&witness.witness_id) {
            ...
            let signature_bytes =
                match base64::engine::general_purpose::STANDARD.decode(&witness.signature) {
                    Ok(bytes) => bytes,
                    Err(e) => {
                        log::warn!(
                            "Invalid signature encoding for witness {}: {}",
                            witness.witness_id,
                            e
                        );
                        continue;
            ...
            let signature = match Signature::try_from(signature_bytes.as_slice()) {
                ...
                Err(e) => {
                    log::warn!(
                        "Invalid signature format for witness {}: {}",
                        witness.witness_id,
                        e
                    );
                    continue;
    ...
    if is_valid {
        log::info!(
            "Witness validation successful: {}/{} valid signatures",
            valid_signatures,
            witnesses.len()
        );
    } else {
        log::warn!(
            "Witness validation failed: {}/{} valid signatures, minimum 3 required",
            valid_signatures,
            witnesses.len()
        );
    }
...
mod tests {
    ...
    fn test_witness_validation() {
        ...
        assert!(
            result,
            "Witness validation should succeed with 3 valid signatures"
        );
    }
    ...
    fn test_insufficient_witnesses() {
        ...
        assert!(
            !result,
            "Witness validation should fail with less than 3 witnesses"
        );
    }
}
