...
impl WitnessRegistry {
    ...
    pub fn validate_witness_signature(
        ...
        if pub_key_bytes.len() != 32 {
            return Err(SigilError::crypto_error(
        ...
        let verifying_key = ed25519_dalek::VerifyingKey::from_bytes(
            pub_key_bytes
                .as_slice()
                .try_into()
                .map_err(|_| SigilError::crypto_error("Failed to convert public key bytes"))?,
        )
        ...
        let signature = ed25519_dalek::Signature::from_bytes(
            sig_bytes
                .as_slice()
                .try_into()
                .map_err(|_| SigilError::crypto_error("Failed to convert signature bytes"))?,
        );
    ...
    pub fn reload_from_canon(&self) -> SigilResult<()> {
        ...
        for record in records {
            match serde_json::from_value::<TrustedWitness>(record.payload) {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to deserialize witness from record {}: {}",
                        record.id,
                        e
                    );
                }
