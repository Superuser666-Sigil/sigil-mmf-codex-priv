...
fn canonical_json_bytes<T: Serialize>(v: &T) -> anyhow::Result<Vec<u8>> {
    // deterministic via serde_json + map ordering by using BTreeMap via intermediate Value
    let val = serde_json::to_value(v).context("serialize license payload")?;
    ...
    serde_json::to_vec(&sort(&val)).context("serialize canonical license bytes")
}
...
fn main() -> anyhow::Result<()> {
    ...
    let pass = a
        .passphrase
        .or_else(|| std::env::var("MMF_LICENSE_KDF_PASSPHRASE").ok())
    ...
    let (sk, pk_b64) = ks
        ...
        .context("failed to load/create root license key")?;
