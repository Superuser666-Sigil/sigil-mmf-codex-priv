...
) -> Result<CanonSigningKey, Box<dyn std::error::Error>> {
    match key_path {
        Some(path) => {
...
fn sign_batch(
    ...
    for entry in fs::read_dir(&input_dir)? {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
            ...
            for (line_num, line) in content.lines().enumerate() {
                ...
                let mut record: CanonicalRecord = serde_json::from_str(line).map_err(|e| {
                    format!("Error parsing {}:{}: {}", path.display(), line_num + 1, e)
                })?;
    ...
    println!(
        "🎉 Batch complete! Signed {} total records",
        total_processed
    );
    println!(
        "🔑 Signed with public key: {}",
...
fn sign_record(
    ...
    let canonical_json = canonical_record
        .to_canonical_json()
    ...
    if computed_hash != record.hash {
        println!(
            "🔄 Updating hash for record {} to JCS-canonical hash",
            record.id
        );
...
fn verify_record(record: &CanonicalRecord) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let verifying_key =
        ed25519_dalek::VerifyingKey::from_bytes(&pub_key_bytes.try_into().unwrap())?;
