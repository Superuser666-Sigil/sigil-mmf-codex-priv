...
use crate::{
    api::{
        ...
        memory::{memory_list, memory_write, rag_upsert},
...
pub fn build_enhanced_router(app_state: Arc<AppState>) -> Router<Arc<AppState>> {
    ...
    let router = Router::new()
        ...
        .layer(middleware::from_fn_with_state(app_state, crate::loa_policy::loa_guard));
...
async fn run_rust_mentor_module(
    ...
    // Run the module
    match module
        .run(req.input, &req.session_id, &req.user_id, &runtime)
        .await
    {
        Ok(output) => Ok(axum::Json(crate::sigilweb::ModuleRunResponse::from_output(
            output,
        ))),
        Err(e) => Ok(axum::Json(crate::sigilweb::ModuleRunResponse::from_error(
            e.to_string(),
        ))),
    }
}
...
async fn bootstrap_root_license(
    ...
    if !bootstrap_enabled {
        return Err(crate::api_errors::AppError::forbidden(
    ...
    let (sk, pk_b64) = ks
        .load_or_create_ed25519("root_license.ed25519.enc", &req.passphrase)
