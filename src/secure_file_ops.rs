...
impl SecureFileOperations {
    ...
    pub fn new(allowed_paths: Vec<String>, max_file_size: usize) -> Result<Self, SigilError> {
        ...
        for path_str in allowed_paths {
            ...
            if !path.exists() {
                return Err(SigilError::validation("allowed_paths", 
        ...
        Ok(SecureFileOperations {
            ...
            allowed_extensions: vec![
                "json".to_string(), "toml".to_string(), "txt".to_string(),
                "log".to_string(), "dat".to_string(), "bin".to_string(),
            ],
        })
    }
    ...
    pub fn validate_path(&self, path: &Path) -> Result<(), SigilError> {
        ...
        if path_str.contains("..") || path_str.contains("~") {
            return Err(SigilError::validation("path", "Path traversal attempt detected"));
        }
    ...
    pub fn read_file_secure(&self, path: &Path) -> Result<Vec<u8>, SigilError> {
        ...
        let metadata = fs::metadata(path)
            .map_err(|e| SigilError::io("getting file metadata", e))?;
        ...
        fs::read(path)
            .map_err(|e| SigilError::io("reading file", e))
    }
    ...
    pub fn write_file_secure(&self, path: &Path, data: &[u8]) -> Result<(), SigilError> {
        ...
        if let Some(parent) = path.parent() {
            ...
            fs::create_dir_all(parent)
                .map_err(|e| SigilError::io("creating directory", e))?;
        }
        
        fs::write(path, data)
            .map_err(|e| SigilError::io("writing file", e))
    }
    ...
    pub fn verify_file_integrity(&self, path: &Path, expected_hash: &str) -> Result<bool, SigilError> {
    ...
    pub fn list_files_secure(&self, dir_path: &Path) -> Result<Vec<PathBuf>, SigilError> {
        ...
        for entry in fs::read_dir(dir_path)
            .map_err(|e| SigilError::io("reading directory", e))? {
    ...
    pub fn move_file_secure(&self, src: &Path, dst: &Path) -> Result<(), SigilError> {
        ...
        fs::rename(src, dst)
            .map_err(|e| SigilError::io("moving file", e))
    }
    ...
    pub fn delete_file_secure(&self, path: &Path) -> Result<(), SigilError> {
        ...
        fs::remove_file(path)
            .map_err(|e| SigilError::io("deleting file", e))
    }
