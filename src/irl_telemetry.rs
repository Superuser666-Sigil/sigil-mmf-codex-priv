...
pub fn log_telemetry(chain: &ReasoningChain) -> std::io::Result<()> {
    ...
    println!("[TELEMETRY] Logged chain with {} reasoning tokens", chain.reasoning.len());
    Ok(())
}
...
impl IRLTelemetry {
    ...
    pub fn record_decision(&self, event: &crate::audit::AuditEvent, score: f32, allowed: bool) {
        let decision_telemetry = IRLTelemetry {
            ...
            data: format!("Event: {} by {} on {}", event.action, event.who, event.target.as_deref().unwrap_or("unknown")),
            chain_id: "".to_string(),
            reasoning_tokens: 0,
            context_tokens: 0,
            trust_score: score,
            verdict: if allowed { "Allow".to_string() } else { "Deny".to_string() },
        };
