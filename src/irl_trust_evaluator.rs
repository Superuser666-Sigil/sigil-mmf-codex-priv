...
impl TrustEvaluator {
    ...
    pub fn evaluate(&self, chain: &ReasoningChain) -> f32 {
        ...
        let model = self.models.get(&self.model_id)
            .unwrap_or_else(|| {
    ...
    pub fn evaluate_event(&self, event: &crate::audit::AuditEvent, model_id: &str) -> Result<(f32, bool), String> {
        // Create a simple reasoning chain from the audit event
        let mut chain = ReasoningChain::new(
            format!("Event: {} on {}", event.action, event.target.as_deref().unwrap_or("unknown")),
            event.loa.clone()
        );
        ...
        let threshold = self.thresholds.get(model_id)
            .unwrap_or(&0.5);
        ...
        chain.set_verdict(if allowed { crate::audit_chain::Verdict::Allow } else { crate::audit_chain::Verdict::Deny });
