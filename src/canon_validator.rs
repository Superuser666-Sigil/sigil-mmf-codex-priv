...
pub fn validate_canon_file(path: &Path) -> Result<(), String> {
    let data = fs::read_to_string(path).map_err(|e| format!("Failed to read canon file: {e}"))?;

    let parsed: Value = serde_json::from_str(&data).map_err(|e| format!("Invalid JSON: {e}"))?;

    let metadata = parsed.get("metadata").ok_or("Missing 'metadata' block")?;

    let entries = parsed.get("entries").ok_or("Missing 'entries' block")?;
    ...
    for (i, entry) in entries_arr.iter().enumerate() {
        ...
        if matches!(
            entry_type,
            "cyberware" | "bioware" | "spell" | "matrix" | "ritual" | "metamagic"
        ) {
            if !has_desc {
                println!(
                    "WARNING: Entry [{}] '{}' lacks a description.",
                    i,
                    entry.get("name").unwrap_or(&json!("(unknown)"))
                );
            }
            if !has_quote {
                println!(
                    "WARNING: Entry [{}] '{}' lacks a flavor quote.",
                    i,
                    entry.get("name").unwrap_or(&json!("(unknown)"))
                );
            }
    ...
    println!(
        "Canon schema validated. Entries: {}, SHA256: {}",
        entries_arr.len(),
        hash
    );
    Ok(())
...
pub fn validate_entry(entry: &serde_json::Value) -> Result<(), String> {
    ...
    if !valid_types.contains(&entry_type) {
        return Err(format!(
            "Invalid entry type '{entry_type}'. Valid types: {valid_types:?}"
        ));
    }
    ...
    if matches!(
        ...
        if entry.get("description").is_none() {
            return Err(format!(
                "Entry '{name}' of type '{entry_type}' should have a description"
            ));
        }

        if entry.get("flavor_quote").is_none() {
            return Err(format!(
                "Entry '{name}' of type '{entry_type}' should have a flavor quote"
            ));
        }
    }
    ...
    if let Some(trust_level) = entry.get("trust_level") {
        if let Some(level) = trust_level.as_str() {
            ...
            if !valid_levels.contains(&level) {
                return Err(format!(
                    "Invalid trust_level '{level}'. Valid levels: {valid_levels:?}"
                ));
            }
        } else {
