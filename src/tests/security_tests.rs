...
fn test_trust_linear_model() {
    ...
    println!("Low-risk features: score={}, allowed={}", low_risk_score, allowed);
    assert!(allowed, "Low-risk operation should be allowed");
    assert!(low_risk_score > 0.4, "Low-risk operation should have high score");
    ...
    println!("High-risk features: score={}, allowed={}, threshold={}", high_risk_score, allowed, threshold);
    ...
    assert!(high_risk_score != low_risk_score, "High-risk and low-risk operations should have different scores");
}
...
fn test_quorum_system() -> SigilResult<()> {
    ...
    let public_key2_b64 = base64::engine::general_purpose::STANDARD
        .encode(signing_key2.verifying_key().as_bytes());
    ...
    let signature1_b64 = base64::engine::general_purpose::STANDARD
        .encode(signature1.to_bytes());
    
    let signature2 = signing_key2.sign(message);
    let signature2_b64 = base64::engine::general_purpose::STANDARD
        .encode(signature2.to_bytes());
