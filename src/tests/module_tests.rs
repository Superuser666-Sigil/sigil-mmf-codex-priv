...
fn test_module_loa_enforcement_operator_success() {
    ...
    assert!(
        result.is_ok(),
        "Operator LOA should be able to execute hello module"
    );
    let output = result.unwrap();
    assert!(
        output.contains("Hello from Sigil!"),
        "Module should return expected greeting"
    );
    assert!(
        output.contains("test_session"),
        "Module should include session ID"
    );
    assert!(
        output.contains("test_user"),
        "Module should include user ID"
    );
    assert!(
        output.contains("Operator"),
        "Module should include LOA level"
    );
}
...
fn test_module_loa_enforcement_guest_denied() {
    ...
    assert!(
        result.is_err(),
        "Guest LOA should be denied access to hello module"
    );
    ...
    match error {
        crate::errors::SigilError::InsufficientLoa { required, actual } => {
            assert_eq!(
                required,
                LOA::Operator,
                "Module should require Operator LOA"
            );
...
fn test_module_registry_builtin_modules() {
    ...
    assert!(
        hello_module.is_some(),
        "Hello module should be registered by default"
    );
    assert_eq!(
        hello_module.unwrap().required_loa(),
        LOA::Operator,
        "Hello module should require Operator LOA"
    );
}
