...
use crate::keys::{KeyManager, KeyStore, CanonSigningKey};
...
fn test_complete_key_lifecycle() {
    ...
    assert_eq!(initial_store.current_key_index(), 0, "new store should start at index 0");
    
    let new_index = initial_store.rotate_key();
    assert_eq!(new_index, 1, "first key should have index 1");
    
    // Step 2: Test key persistence
    initial_store.save_to_directory(&key_dir, &encryption_key)
        .expect("should save key store");
    
    // Step 3: Test key loading
    let loaded_store = KeyStore::load_from_directory(&key_dir, &encryption_key)
        .expect("should load key store");
    
    assert_eq!(loaded_store.current_key_index(), 1, "loaded store should match");
    ...
    assert_ne!(old_key_pubkey, new_key_pubkey, "rotated key should be different");
    
    // Step 5: Test historical key access
    let historical_key = rotating_store.get_key(1).expect("should access historical key");
    assert_eq!(historical_key.public_key_b64(), old_key_pubkey, "historical key should match original");
...
fn test_key_encryption_security() {
    ...
    // Save encrypted
    test_key.save_encrypted(&key_path, &encryption_key, 1, "test key")
        .expect("should save encrypted");
    
    // Verify encrypted format
    let file_contents = std::fs::read_to_string(&key_path).expect("should read file");
    assert!(file_contents.contains("encrypted_private_key_b64"), "should use encrypted format");
    assert!(file_contents.contains("nonce_b64"), "should contain AES-GCM nonce");
    ...
    assert_eq!(test_key.public_key_b64(), reloaded_key.public_key_b64(), "public key should match");
    ...
    test_key.verify_signature(test_data, &reloaded_sig.0).expect("should cross-verify");
    reloaded_key.verify_signature(test_data, &original_sig.0).expect("should cross-verify");
...
fn test_key_manager_integration() {
    ...
    let encryption_key_b64 = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &test_encryption_key);
    ...
    assert_eq!(retrieved_key, test_encryption_key, "should use environment key");
    
    let key_directory = KeyManager::get_key_directory();
    assert_eq!(key_directory, custom_key_dir, "should use environment directory");
    ...
    canon_key.verify_signature(test_message, &signature.0).expect("should verify");
