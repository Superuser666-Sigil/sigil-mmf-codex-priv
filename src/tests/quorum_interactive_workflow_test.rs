...
fn test_interactive_quorum_workflow_full_cycle() {
    ...
    let mentor_keys: Vec<_> = (0..3).map(|i| {
        ...
        runtime.witness_registry.add_witness(
            witness_id.clone(),
            public_key_b64,
    ...
    let proposal_id = {
        let mut quorum_system = runtime.quorum_system.lock().expect("should lock quorum system");
        quorum_system.create_proposal(
    ...
    let initial_status = {
        let quorum_system = runtime.quorum_system.lock().expect("should lock quorum system");
        quorum_system.get_proposal(&proposal_id).expect("proposal should exist").clone()
    };
    
    assert_eq!(initial_status.get_signature_count(), 0, "should start with 0 signatures");
    assert!(!initial_status.has_quorum(), "should not have quorum initially");
    assert!(!initial_status.is_expired(), "should not be expired initially");
    
    println!("✅ Initial status verified: 0/{} signatures", initial_status.required_k);
    ...
    for (i, (witness_id, signing_key)) in mentor_keys.iter().enumerate() {
        ...
        assert_eq!(current_status.get_signature_count(), expected_signatures, 
                   "should have {} signatures after mentor {}", expected_signatures, i + 1);
        
        if expected_signatures >= current_status.required_k {
            assert!(current_status.has_quorum(), 
                    "should have quorum with {}/{} signatures", 
                    expected_signatures, current_status.required_k);
            println!("   🎯 Quorum reached! {}/{} signatures", 
                     expected_signatures, current_status.required_k);
        } else {
            assert!(!current_status.has_quorum(), 
                    "should not have quorum with {}/{} signatures", 
                    expected_signatures, current_status.required_k);
            println!("   ⏳ Partial quorum: {}/{} signatures", 
                     expected_signatures, current_status.required_k);
        }
    }
    ...
    assert!(final_proposal.has_quorum(), "final proposal should have quorum");
    assert_eq!(final_proposal.get_signature_count(), 3, "should have all 3 signatures");
    ...
    let committed_proposal = {
        let mut quorum_system = runtime.quorum_system.lock().expect("should lock quorum system");
        quorum_system.commit_proposal(&proposal_id).expect("should commit proposal")
    };
    ...
    println!("🔐 Persisting to Canon with witness signatures");
    {
        ...
        canon_store.add_record(record.clone(), &LOA::Root, true)
    ...
    let reloaded_record = {
        ...
        canon_store.load_record(&committed_proposal.entry, &LOA::Root)
    ...
    signing_key.verify_signature(reloaded_canonical.as_bytes(), reloaded_signature)
    ...
    for (i, witness_record) in reloaded_record.witnesses.iter().enumerate() {
        ...
        assert!(mentor_keys.iter().any(|(witness_id, _)| witness_id == &witness_record.witness_id),
                "witness {} should be from registered mentors", i);
    }
    
    // Step 10: Verify proposal is no longer in pending list
    println!("🧹 Verifying proposal cleanup");
    let quorum_system = runtime.quorum_system.lock().expect("should lock quorum system");
    assert!(quorum_system.get_proposal(&proposal_id).is_none(), 
...
fn test_proposal_status_tracking() {
    ...
    let runtime = SigilRuntimeCore::new(LOA::Root, canon_store, config)
        .expect("should create runtime");
    ...
    {
        let mut quorum_system = runtime.quorum_system.lock().expect("should lock quorum system");
        quorum_system.add_signature(&proposal_id, "test_witness_1".to_string(), signature_b64)
    ...
    {
        let mut quorum_system = runtime.quorum_system.lock().expect("should lock quorum system");
        let _committed = quorum_system.commit_proposal(&proposal_id)
...
fn test_partial_quorum_failure_scenarios() {
    ...
    let witness_keys: Vec<_> = (0..2).map(|i| {
        let signing_key = SigningKey::generate(&mut rand::rngs::OsRng);
        let public_key_b64 = base64::engine::general_purpose::STANDARD
            .encode(signing_key.verifying_key().as_bytes());
    ...
    let content_hash = {
        let quorum_system = runtime.quorum_system.lock().expect("should lock quorum system");
        quorum_system.get_proposal(&proposal_id).expect("proposal should exist").content_hash.clone()
    };
    ...
    assert_eq!(partial_status.get_signature_count(), 2, "should have 2 signatures");
    assert!(!partial_status.has_quorum(), "should not have quorum (2/3)");
    assert_eq!(partial_status.get_remaining_signatures_needed(), 1, "should need 1 more signature");
    ...
    assert!(error_msg.contains("requires 3 signatures"), "error should mention signature requirement");
    
    println!("✅ Verified commit failure without quorum");
    
    // Verify proposal remains in pending state
    let quorum_system = runtime.quorum_system.lock().expect("should lock quorum system");
    assert!(quorum_system.get_proposal(&proposal_id).is_some(),
