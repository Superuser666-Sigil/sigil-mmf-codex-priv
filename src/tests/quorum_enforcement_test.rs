...
fn test_quorum_enforcement_blocks_system_writes_without_quorum() {
    ...
    let public_key1_b64 = base64::engine::general_purpose::STANDARD
        .encode(signing_key1.verifying_key().as_bytes());
    ...
    quorum_system.add_signature(&proposal_id, "test_mentor_1".to_string(), signature_b64)
        .expect("should add signature");
    
    // Verify that quorum is NOT reached
    let proposal = quorum_system.get_proposal(&proposal_id).expect("proposal should exist");
    assert!(!proposal.has_quorum(), "should not have quorum with only 1 of 3 required signatures");
...
fn test_quorum_enforcement_allows_system_writes_with_quorum() {
    ...
    let proposal = quorum_system.get_proposal(&proposal_id).expect("proposal should exist");
    assert!(proposal.has_quorum(), "should have quorum with 3 of 3 required signatures");
    
    // Commit should succeed
    let committed_proposal = quorum_system.commit_proposal(&proposal_id)
    ...
    assert_eq!(committed_proposal.content, "critical_system_setting=new_value");
    assert_eq!(committed_proposal.signers.len(), 3);
    
    // Verify the proposal is no longer in the pending list
    assert!(quorum_system.get_proposal(&proposal_id).is_none(), "committed proposal should be removed from pending");
...
fn test_user_space_writes_bypass_quorum() {
    ...
    // Set up encrypted canon store
    let encryption_key = KeyManager::get_encryption_key().expect("should get encryption key");
    let mut canon_store = EncryptedCanonStoreSled::new(path, &encryption_key)
        .expect("should create encrypted store");
    ...
    assert!(result.is_ok(), "user space write should succeed without quorum");
