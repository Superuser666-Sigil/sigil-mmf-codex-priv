...
impl SigilModule for TestModule {
    ...
    fn run(&self, ctx: &ModuleContext) -> SigilResult<String> {
        Ok(format!("{}: Executed with LOA {:?} for user {} in session {}", 
...
impl SigilModule for ResourceIntensiveModule {
    ...
    fn run(&self, ctx: &ModuleContext) -> SigilResult<String> {
        ...
        Ok(format!("Resource-intensive module completed {} iterations, result: {}", iterations, sum))
    }
}
...
impl SigilModule for FallibleModule {
    ...
    fn run(&self, ctx: &ModuleContext) -> SigilResult<String> {
        match ctx.input.as_str() {
            ...
            _ => Ok(format!("Fallible module succeeded with input: {}", ctx.input))
        }
    }
}
...
fn test_comprehensive_loa_enforcement() {
    ...
    // Register test modules with different LOA requirements
    {
        ...
        module_registry.register_module("guest_module", Box::new(TestModule::new(LOA::Guest, "Guest")));
        module_registry.register_module("observer_module", Box::new(TestModule::new(LOA::Observer, "Observer")));
        module_registry.register_module("operator_module", Box::new(TestModule::new(LOA::Operator, "Operator")));
        module_registry.register_module("mentor_module", Box::new(TestModule::new(LOA::Mentor, "Mentor")));
        module_registry.register_module("root_module", Box::new(TestModule::new(LOA::Root, "Root")));
    }
    ...
    for (user_loa, module_name, should_succeed) in test_cases {
        ...
        if should_succeed {
            assert!(result.is_ok(), 
                    "LOA {:?} should be able to execute {}", user_loa, module_name);
            let output = result.unwrap();
            assert!(output.contains(&format!("{:?}", user_loa)), 
                    "Output should contain user LOA level");
        } else {
            assert!(result.is_err(), 
                    "LOA {:?} should NOT be able to execute {}", user_loa, module_name);
...
fn test_module_error_handling() {
    ...
    for (input, expected_error) in error_cases {
        ...
        assert!(error_msg.contains(expected_error), 
                "Error should contain expected message for input: {}", input);
    }
    ...
    assert!(result.unwrap().contains("succeeded"), "Should contain success message");
...
fn test_module_registry_management() {
    ...
    
    {
        ...
        assert!(module_registry.get_module("test1").is_some(), "Should find registered module test1");
        assert!(module_registry.get_module("test2").is_some(), "Should find registered module test2");
        assert!(module_registry.get_module("nonexistent").is_none(), "Should not find nonexistent module");
        
        // Test built-in module exists
        assert!(module_registry.get_module("hello").is_some(), "Should have built-in hello module");
        
        // Test module overwriting
        module_registry.register_module("test1", Box::new(TestModule::new(LOA::Root, "Test1Updated")));
        let updated_module = module_registry.get_module("test1").unwrap();
        assert_eq!(updated_module.required_loa(), LOA::Root, "Module should be updated");
    }
...
fn test_module_context_validation() {
    ...
    for (input, should_succeed) in test_inputs {
        ...
        if should_succeed {
            ...
            let output = result.unwrap();
            assert!(output.contains("completed"), "Should contain completion message");
        }
    }
...
fn test_module_isolation() {
    ...
    {
        let mut module_registry = runtime.module_registry.lock().unwrap();
        module_registry.register_module("module_a", Box::new(TestModule::new(LOA::Guest, "ModuleA")));
        module_registry.register_module("module_b", Box::new(TestModule::new(LOA::Guest, "ModuleB")));
    }
    ...
    let results: Vec<_> = contexts.iter().enumerate().map(|(i, ctx)| {
