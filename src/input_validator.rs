...
impl InputValidator {
    ...
    pub fn validate_trust_request(&self, req: &TrustCheckRequest) -> SigilResult<()> {
        ...
        if self.contains_injection_patterns(&req.who)
            ...
            || req
                .target
                .as_ref()
                .is_some_and(|t| self.contains_injection_patterns(t))
        {
            return Err(crate::errors::SigilError::validation(
                "input",
    ...
    pub fn validate_cli_input(&self, command: &CliCommand) -> SigilResult<()> {
        match command {
            ...
            CliCommand::RegisterExtension { name, loa } => {
                ...
                LOA::from_str(loa).map_err(|_| {
                    crate::errors::SigilError::validation("loa", "Invalid LOA level")
                })?;
            }
        }
    ...
    pub fn validate_extension_registration(
        &self,
