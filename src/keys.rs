...
impl KeyStore {
    ...
    pub fn current_key(&self) -> Result<&CanonSigningKey, KeyError> {
        self.keys.get(&self.current_key_index)
            .ok_or_else(|| KeyError::KeyNotFound(format!("current key index {}", self.current_key_index)))
    }
    ...
    pub fn load_from_directory<P: AsRef<Path>>(dir_path: P, encryption_key: &[u8; 32]) -> Result<Self, KeyError> {
        ...
        if store.keys.is_empty() {
            return Err(KeyError::KeyNotFound("No valid keys found in directory".to_string()));
        }
    ...
    pub fn save_to_directory<P: AsRef<Path>>(&self, dir_path: P, encryption_key: &[u8; 32]) -> Result<(), KeyError> {
...
impl CanonSigningKey {
    ...
    pub fn save_encrypted<P: AsRef<Path>>(&self, path: P, encryption_key: &[u8; 32], key_index: u32, purpose: &str) -> Result<(), KeyError> {
        ...
        let encrypted_private_key = cipher.encrypt(nonce, private_key_bytes.as_ref())
            .map_err(|e| KeyError::Encryption(e.to_string()))?;
    ...
    pub fn load_encrypted<P: AsRef<Path>>(path: P, encryption_key: &[u8; 32]) -> Result<(Self, u32), KeyError> {
        let content = fs::read_to_string(path)?;
        ...
        let sk_array: [u8; 32] = private_key_bytes.try_into()
    ...
    pub fn verify_signature(&self, canonical_bytes: &[u8], signature_b64: &str) -> Result<(), KeyError> {
...
impl KeyManager {
    ...
    pub fn get_encryption_key() -> Result<[u8; 32], KeyError> {
        ...
        log::warn!("No CANON_ENCRYPTION_KEY environment variable set, using default key for development");
    ...
    pub fn get_or_create_key_store() -> Result<KeyStore, KeyError> {
        ...
        if key_dir.exists() {
            match KeyStore::load_from_directory(&key_dir, &encryption_key) {
                ...
                Err(e) => {
                    log::warn!("Failed to load encrypted key store: {}, creating new one", e);
                }
            }
        }
    ...
    pub fn verify_key_store(store: &KeyStore) -> Result<(), KeyError> {
        for &key_index in &store.key_indices() {
            ...
            Self::verify_key_pair(key)
                .map_err(|e| KeyError::Encryption(format!("Key {} verification failed: {}", key_index, e)))?;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_encrypted_key_storage() {
        ...
        // Save encrypted
        original_key.save_encrypted(&key_path, &encryption_key, 1, "test key").unwrap();
        
        // Load encrypted
        let (loaded_key, key_index) = CanonSigningKey::load_encrypted(&key_path, &encryption_key).unwrap();
    ...
    fn test_key_store_rotation() {
        ...
        assert_ne!(historical_key.public_key_b64(), current_key.public_key_b64());
    ...
    fn test_key_store_directory_operations() {
        ...
        // Save to directory
        original_store.save_to_directory(&key_dir, &encryption_key).unwrap();
        ...
        assert_eq!(loaded_store.current_key_index(), original_store.current_key_index());
        assert_eq!(loaded_store.current_key().unwrap().public_key_b64(), original_current_pk);
    ...
    fn test_rotation_preserves_historical_verification() {
        ...
        assert!(historical_key.verify_signature(test_data, &sig1_b64).is_ok());
        assert!(current_key.verify_signature(test_data, &sig2_b64).is_ok());
        
        // Cross-verification should fail
        assert!(historical_key.verify_signature(test_data, &sig2_b64).is_err());
